...
export async function initLearnPage() {
  ...
  startBtn.addEventListener('click', async () => {
    ...
    await recognizer.init((status) => (overlayEl.textContent = status));
    ...
    recognizer.onResults = (result) => {
      ...
      if (!result.detected) {
        scoreEl.textContent = 'Show your hand to the camera';
        holdFrames = 0;
        return;
      }

      drawLandmarks(ctx, result.landmarks, canvasEl.width, canvasEl.height);
